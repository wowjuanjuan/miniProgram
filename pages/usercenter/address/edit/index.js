...
Page({
  ...
  onLoad(options) {
    ...
    // console.log(JSON.parse(tempAddress));
    if (tempAddress) {
      this.init(JSON.parse(tempAddress));
    } else {
      this.init()
    }
  },
  ...
  init(oldAddress) {
    if (oldAddress) {
      this.getAddressDetail(oldAddress);
    }
  },
  getAddressDetail(oldAddress) {
    this.setData({
      locationState: oldAddress
    }, () => {
      const {
        isLegal,
        tips
  ...
  onCheckDefaultAddress({
    ...
    console.log(this.data.locationState.isDefault);
  },
